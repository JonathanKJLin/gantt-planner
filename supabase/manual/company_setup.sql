-- Company setup SQL for Resource Gantt Planner
--
-- Use this file for a NEW company Supabase project.
-- It is intentionally self-contained and excludes historical Notion pull
-- experiments (`notion_pull_*`, Vault/http based pull, pg_cron). The production
-- pull path is GitHub Action -> scripts/notion-sync/sync.mjs -> notion_page_cache.
--
-- After running this file:
-- 1. Update public.sync_config for the company Notion database and property names.
-- 2. Update index.html with the new Supabase URL + anon key.
-- 3. Add GitHub Actions secrets (SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, NOTION_TOKEN).
-- 4. Run the GitHub Action once with full_resync=true.
...
grant select, update on public.gantt_state to anon, authenticated;
grant select, insert, delete on public.gantt_history to anon, authenticated;

-- ---------------------------------------------------------------------------
-- Sync config, logs, status
-- ---------------------------------------------------------------------------
...
insert into public.sync_config (key, value) values
  ('notion_database_id', 'REPLACE_WITH_COMPANY_NOTION_DATABASE_ID'),
  ...
  ('gantt_landscape_allow', '["Go-to-Market","Sprint/Pipelines","Projects - Mtel2","Projects - On-going ( BIA & DMG )","Projects - On-going ( AiLab & RA )"]'),
  ('gantt_require_dates', 'off')
...
-- ---------------------------------------------------------------------------
-- GitHub Action incremental sync checkpoint
-- ---------------------------------------------------------------------------

create table if not exists public.notion_sync_cursor (
  id                 integer primary key default 1 check (id = 1),
  last_edited_synced timestamptz,
...
-- ---------------------------------------------------------------------------
-- Raw Notion page cache filled by scripts/notion-sync/sync.mjs
-- ---------------------------------------------------------------------------
...
grant all on public.notion_page_cache to service_role, postgres;

-- ---------------------------------------------------------------------------
-- Notion JSON helper functions
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Main cache -> gantt_state rebuild
-- ---------------------------------------------------------------------------
...
begin
  ...
  v_prop_date := nullif(trim(coalesce(public.notion_cfg('prop_date'), '')), '');
  v_prop_tag := coalesce(public.notion_cfg('prop_tag'), 'Tag');
  v_prop_parent := coalesce(public.notion_cfg('prop_parent'), 'Parent-task');
  ...
  v_date_mode := lower(coalesce(nullif(trim(public.notion_cfg('gantt_require_dates')), ''), 'both'));

  if v_db_id is null or v_db_id = 'REPLACE_WITH_COMPANY_NOTION_DATABASE_ID' then
  ...
  if v_cache_count = 0 then
    raise exception 'notion_page_cache is empty — run the GitHub Action sync worker first';
  end if;
  ...
  where n.page_id not in (select page_id from _child_ids)
    and not public.notion_tag_is(n.props, v_prop_tag, v_tag_landscape);
  ...
  project_pick as (
    ...
    from chain
    where public.notion_tag_matches(tag, v_tag_project)
  ...
  landscape_pick as (
    ...
    from chain
    where public.notion_tag_matches(tag, v_tag_landscape)
  ...
  path_build as (
    select
      ...
      string_agg(c.title, ' › ' order by c.depth desc) filter (
        where c.depth > 0
          and not public.notion_tag_matches(c.tag, v_tag_landscape)
          and not public.notion_tag_matches(c.tag, v_tag_project)
      ) as wbs_path
    from chain c
    group by c.leaf_id
  )
  ...
  if v_prop_date is not null and v_date_mode not in ('off', 'none', 'false', 'no') then
    if v_date_mode = 'start' then
      delete from _leaf_ctx where date_start is null;
    else
      delete from _leaf_ctx
      where date_start is null
         or nullif(props -> v_prop_date -> 'date' ->> 'end', '') is null;
    end if;
  end if;
  ...
  set finished_at = now(),
      ...
      error_message = case
        when jsonb_array_length(v_tasks) = 0 then
          format('0 tasks from %s cached pages — check prop_date / gantt_require_dates / gantt_landscape_allow', v_cache_count)
        else null
      end
  where id = v_log_id;
...
-- Optional: enable Realtime for frontend updates. If this fails because the
-- table is already in the publication, it is safe to ignore.
do $$
begin
