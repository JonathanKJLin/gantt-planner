-- Make scoped Notion pulls strict and self-validating.
--
-- Previous formula filter used:
--   Ultimate Parent formula string contains '<root>'
--
-- For a large company database this is too loose for diagnostics and makes it
-- hard to distinguish fresh results from old cache rows. This version uses:
--   Ultimate Parent formula string equals '<root>'
--
-- and ALSO guards every cache insert in SQL, so even if a stale/incorrect row
-- appears in the response it will not be written unless its formula string is
-- exactly the requested root title. The root row itself is still cached.
...
declare
  ...
  v_root_title text := trim(p_root_title);
  v_root_page_id text;
  v_filter jsonb;
  v_page jsonb;
  v_page_id text;
  v_page_ultimate text;
  v_added int := 0;
  v_skipped int := 0;
  v_new_rows int;
  v_prog record;
begin
  ...
  if v_root_title is null or v_root_title = '' then
  ...
  if p_reset then
    delete from public.notion_pull_progress where root_title = v_root_title;
    delete from public.notion_pull_seen where root_title = v_root_title;

    -- Cache the root row itself by exact Task Name.
    v_filter := jsonb_build_object(
      'property', v_prop_title,
      'title', jsonb_build_object('equals', v_root_title)
    );
    ...
    if v_status >= 400 then
      raise exception 'Notion API error % (find root "%", prop_title=%): %',
        v_status, v_root_title, v_prop_title, v_content;
    end if;

    v_body := v_content::jsonb;
    v_results := coalesce(v_body->'results', '[]'::jsonb);
    if jsonb_array_length(v_results) = 0 then
      raise exception 'Root not found: "%" (prop_title=%)', v_root_title, v_prop_title;
    end if;
    ...
    values (v_root_title, v_root_page_id)
    on conflict do nothing;

    insert into public.notion_pull_progress (
      root_title, root_page_id, strategy, next_cursor, done, pages_fetched
    ) values (
      v_root_title, v_root_page_id, 'ultimate_parent_formula_equals', null, false, 1
    );

    return jsonb_build_object(
      'status', 'continue',
      'root', v_root_title,
      'strategy', 'ultimate_parent_formula_equals',
      'pages_added', 1,
      'pages_skipped', 0,
      'pages_fetched', 1,
  ...
  where root_title = v_root_title;

  if not found then
    raise exception 'No progress for "%". Call notion_pull_step(title, true, clear_cache) first.', v_root_title;
  end if;
  ...
  -- Strict Notion API filter: formula string must exactly equal root title.
  v_filter := jsonb_build_object(
    'property', v_prop_ultimate,
    'formula', jsonb_build_object(
      'string', jsonb_build_object('equals', v_root_title)
    )
  );

  v_body_obj := jsonb_build_object('filter', v_filter, 'page_size', 100);
  ...
  if v_status >= 400 then
    raise exception 'Notion API error % (Ultimate Parent formula equals filter, property=%): %',
  ...
  for v_page in select value from jsonb_array_elements(v_results) loop
    ...
    v_page_ultimate := v_page->'properties'->v_prop_ultimate->'formula'->>'string';

    -- Second line of defense: never cache a row outside the requested root.
    if coalesce(v_page_ultimate, '') <> v_root_title then
      v_skipped := v_skipped + 1;
      continue;
    end if;
