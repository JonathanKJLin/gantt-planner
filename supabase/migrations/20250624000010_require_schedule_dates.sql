-- Only show tasks that actually have schedule dates on the Gantt.
--
-- Context: tasks in Notion previously had no real start/end dates (only a "sync
-- date"), so the timeline was empty. The admin is adding a proper Date property
-- (one Notion "Date" property with End date enabled = start + end in one field).
--
-- This migration adds a date filter to sync_gantt_from_notion() driven by
-- sync_config.gantt_require_dates:
--   'both'  (default) -> require an explicit START and END date
--   'start'           -> require a START only (end falls back to start = 1-day bar)
--   'off' / ''        -> no date filter (old behaviour)
--
-- IMPORTANT: point prop_date at the new Notion property, e.g.
--   update public.sync_config set value = 'Gantt Date' where key = 'prop_date';
--
-- Body is identical to 20250624000009 plus:
--   1. v_date_mode declaration
--   2. read gantt_require_dates from sync_config
--   3. delete leaves without the required dates from _leaf_ctx
...
declare
  ...
  v_allow_count int := 0;
  v_date_mode text;           -- 'both' | 'start' | 'off'
  v_existing jsonb;
...
begin
  ...
  -- ⓶ Optional date requirement: only show tasks that have schedule dates.
  --   'both' (default) = explicit start AND end; 'start' = start only; 'off' = no filter
...
-- Default: only show tasks with both a start and end date. Set to 'start' to
-- allow start-only (1-day) tasks, or '' / 'off' to disable the date filter.
insert into public.sync_config (key, value)
values ('gantt_require_dates', 'both')
